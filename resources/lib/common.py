...
subtitlepath = os.path.join(datapath,'SubTitles')
...
def postSTOP( type,content_id,position ):
    log( 'HULU --> common :: postSTOP :: content_id = '+content_id)
...
def login_queue():
    if settings['login_name']=='' or settings['login_pass']=='':
        log( "Hulu --> WARNING: Could not login.  Please enter a username and password in settings")
        return False
    action = "authenticate"
...
def viewed(videoid):
    try:
        ...
        parameters = {'event_type':'view',
                      ...
                      'id':videoid}
        postAPI(action,parameters,False)
        log( "HULU --> Posted view")
    except:
        log( "HULU --> Post view failed")
...
def login_cookie():
    ...
    log( 'HULU -- > These are the cookies we have received:')
    for index, cookie in enumerate(cj):
        log( 'HULU--> '+str(index)+': '+str(cookie))
        
    log( "HULU --> login_url response (we want 'ok=1'): " + response)
    if response == 'ok=1':
