...
def log(txt):
       message = '%s: %s' % ("HULU : xbmclibrary.py :", txt)
...
if (common.settings['enablelibraryfolder'] == 'true'):
    MOVIE_PATH = os.path.join(xbmc.translatePath('special://profile/addon_data/plugin.video.hulunew/'),'Movies')
    TV_SHOWS_PATH = os.path.join(xbmc.translatePath('special://profile/addon_data/plugin.video.hulunew/'),'TV')
...
class Main:
    ...
    def ExportShowMovie(self, item):
        ...
        if feature_films_count == '1' or feature_films_count == 1:
            ...
            strm = 'plugin://plugin.video.hulunew/?mode="TV_play"'
...
#         log( "tvshowDetails = "+tvshowDetails)
    ...
    def SetupHuluLibrary(self):
        log( "Trying to add Hulu source paths...")
